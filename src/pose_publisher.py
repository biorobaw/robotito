...
if __name__ == "__main__":
  ...
  while not rospy.is_shutdown():
    ...
    latest_cam2 = None
    try:
      latest_cam1 = listener.getLatestCommonTime(map_frame, marker_frame)
    except (tf.Exception):
      pass

    try:
      latest_cam2 = listener.getLatestCommonTime(map_frame, marker_frame)
    except (tf.Exception):
      pass
	...
	if (latest_cam1 != None):
	  ...
	  time_to_use = latest_cam2
        (trans,rot) = listener.lookupTransform(map_frame, marker_frame,  time_to_use)
